...
class _ConsultationDetailScreenState extends State<ConsultationDetailScreen>
  ...
  List<Map<String, dynamic>> get _medicines {
    // Try 1: from prescriptionText.medicines
    try {
      final raw = widget.consultation['prescriptionText'];
      if (raw != null &&
          raw.toString().isNotEmpty &&
    ...
    // Try 2: from prescription field directly
    try {
  ...
  void initState() {
    ...
    _checkUploadStatus(); // ← check real status from DB
  }
...
$followUp''';
  ...
  Future<void> _translateFullOPD(String language) async {
    ...
    if (language == 'English') {
      setState(() => _translatedOpdBlock = '');
      return;
    }

    if (_cache.containsKey(language)) {
      setState(() => _translatedOpdBlock = _cache[language]!);
      return;
    }
    ...
    try {
      ...
      final prompt =
      '''Translate the following medical OPD report completely to $language language.
Translate ALL content including section headings.
Keep the same format with each section on a new line.
...
$englishBlock''';
      ...
      if (response.statusCode == 200) {
        ...
        if (candidates != null && candidates.isNotEmpty) {
          ...
          _cache[language] = text.trim();
          setState(() {
            _translatedOpdBlock = text.trim();
            _translating = false;
          });
          return;
        }
    ...
    setState(() {
      _translatedOpdBlock = _buildEnglishOpdBlock();
      _translating = false;
    });
  }
      ...
      pdf.addPage(pw.Page(
        ...
        build: (_) => pw.Column(
          ...
          children: [
            ...
            pw.Text(pdfContent,
                style: pw.TextStyle(
                    font: font, fontSize: 12, lineSpacing: 6)),
            pw.Spacer(),
      ...
      body: Column(children: [
        ...
        Container(
          ...
          child: Column(
              ...
              children: [
                _headerRow(Icons.person, 'Patient',
                    '$patientName  ($patientId)'),
                const SizedBox(height: 4),
                _headerRow(Icons.medical_services, 'Doctor',
                    '$doctorName  ($doctorId)'),
                const SizedBox(height: 4),
        ...
        Expanded(
          child: TabBarView(
            ...
            children: [
                  ...
                  children: [
                    ...
                    Align(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        onPressed:
                        _translating ? null : _downloadOpdPdf,
                      ),
                    ),
                    const SizedBox(height: 12),
                    ...
                    // English structured view
                    else ...[
                        _opdField('🩺 Chief Complaint',
                        ...
                        _opdField('🔬 Observations',
                            o['observations']?.toString()),
                        _opdField(
                            '🧠 Diagnosis',
                        ...
                        _opdField(
                            '💊 Advice',
                            o['advice']?.toString() ??
                                widget.consultation['advice']
                                    ?.toString()),
                        _opdField(
                            '📅 Follow Up',
                            o['follow_up']?.toString() ??
                                widget.consultation['followUp']
                                    ?.toString()),
                      ],
                    ...
                    if (isDoctor) ...[
                      ...
                      // Upload to Government card
                      _uploadOptionCard(
                        icon: Icons.account_balance,
                        iconColor: Colors.blue,
                        title: 'Upload to Government Portal',
                        subtitle:
                        'Anonymized data for health analytics.',
                        ...
                        onTap:
                        _govtDone ? null : _uploadToGovt,
                      ),

                      const SizedBox(height: 10),

                      // Upload to Students card
                      _uploadOptionCard(
                        icon: Icons.menu_book,
                        iconColor: Colors.orange,
                        title: 'Upload for Medical Students',
                        subtitle:
                        'Visible to students for 3 months only.',
                        buttonLabel: _studentsDone
                            ? '✓ Already Uploaded for Students'
                            : 'Upload for Students',
                        buttonColor: _studentsDone
                            ? Colors.grey
                            : Colors.orange,
              ...
              // ════════════════════════════════════════════════════════
              // TAB 2 — PRESCRIPTION (always English)
              // ════════════════════════════════════════════════════════
              SingleChildScrollView(
                padding: const EdgeInsets.all(16),
                child: Column(
  ...
  Widget _opdField(String label, String? value,
